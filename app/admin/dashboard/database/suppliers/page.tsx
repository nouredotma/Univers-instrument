...
export default function SuppliersPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/fournisseurs/${id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(
          data?.message || "Impossible de supprimer le fournisseur",
        );
      }
      await loadSuppliers();
      if (editingId === id) {
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-white">Fournisseurs</h1>
            <p className="text-xs text-gray-400">
              Consulter, créer et modifier les fiches fournisseurs.
            </p>
          </div>
        </div>
            ...
            <tbody>
              {suppliers.length === 0 && !loading ? (
                ...
                suppliers.map((f) => (
                  ...
                  >
                    <td className="px-3 py-2 text-gray-100">{f.nom}</td>
                    <td className="px-3 py-2 text-gray-100">
                      {f.email || <span className="text-gray-500">—</span>}
                    </td>
                    <td className="px-3 py-2 text-gray-100">
                      {f.telephone || (
                    ...
                    <td className="px-3 py-2 text-gray-100">
                      {f.ville || <span className="text-gray-500">—</span>}
                    </td>
                    <td className="px-3 py-2 text-gray-100">
                      {f.adresse || <span className="text-gray-500">—</span>}
                    </td>
