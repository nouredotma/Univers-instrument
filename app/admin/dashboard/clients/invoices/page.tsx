...
export default function ClientInvoicesPage() {
  ...
  const [bls, setBls] = useState<BLClient[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState({ numero: "", date_facture: "", client_id: "", bl_id: "", statut: "" });
  const [lignes, setLignes] = useState<LigneForm[]>([]);

  const resetForm = () => {
    setEditingId(null);
    setForm({ numero: "", date_facture: "", client_id: "", bl_id: "", statut: "" });
    setLignes([]);
  };
  ...
  const handleEdit = (d: FactureClient) => {
    setEditingId(d.id);
    setForm({
      numero: d.numero,
    ...
    const withLines = d as FactureClient & { lignes?: LigneDoc[] };
    setLignes(
  ...
  const handleDelete = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/factures-clients/${id}`, { method: "DELETE" });
  ...
  const handlePrint = (d: FactureClient) => {
    const withLines = d as FactureClient & { lignes?: LigneDoc[] };
    const doc = new jsPDF();
    let y = 20;
    doc.setFontSize(14);
    doc.text("Facture client", 20, y);
    y += 10;
    doc.setFontSize(10);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId ? `${API_BASE_URL}/factures-clients/${editingId}` : `${API_BASE_URL}/factures-clients`;
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-white">Factures client</h1>
            <p className="text-xs text-gray-400">Créer et gérer les factures clients.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <select name="bl_id" value={form.bl_id} onChange={handleInputChange} className="w-full rounded-md bg-[#3a3a3a] border border-white/10 px-3 py-1.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-[#f2762b]">
              ...
              {bls.filter((b) => !form.client_id || b.client_id === form.client_id).map((b) => (
