...
type CommandeClient = {
  id: string; numero: string; date_commande: string; client_id: string;
  devis_id?: string | null; montant_total: string; statut: string;
  client?: Client; devis?: DevisClient; lignes?: LigneDoc[];
};
...
export default function ClientOrdersPage() {
  ...
  const resetForm = () => { setEditingId(null); setForm({ numero: "", date_commande: "", client_id: "", devis_id: "", statut: "" }); setLignes([]); };
  ...
  const loadData = async () => {
    ...
    try {
      const [r1, r2, r3, r4] = await Promise.all([fetch(`${API}/commandes-clients`), fetch(`${API}/clients`), fetch(`${API}/produits`), fetch(`${API}/devis-clients`)]);
      if (!r1.ok || !r2.ok || !r3.ok || !r4.ok) throw new Error("Erreur chargement");
      setItems(await r1.json()); setClients(await r2.json()); setProduits(await r3.json()); setDevis(await r4.json());
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(d => d.numero.toLowerCase().includes(q) || d.client?.nom?.toLowerCase().includes(q) || d.statut?.toLowerCase().includes(q));
  }, [items, searchQuery]);
  ...
  const handleEdit = (d: CommandeClient) => {
    setEditingId(d.id);
    setForm({ numero: d.numero, date_commande: d.date_commande?.slice(0, 10) ?? "", client_id: d.client_id, devis_id: d.devis_id ?? "", statut: d.statut || "" });
  ...
  const handleDelete = async (d: CommandeClient) => {
    setError(null);
    try { const res = await fetch(`${API}/commandes-clients/${d.id}`, { method: "DELETE" }); if (!res.ok) throw new Error("Erreur suppression"); await loadData(); if (editingId === d.id) resetForm(); } catch (e: any) { setError(e.message); }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = { numero: form.numero, date_commande: form.date_commande, client_id: form.client_id, devis_id: form.devis_id || undefined, montant_total: montant, statut: form.statut || "brouillon", lignes: lp.length ? lp : undefined };
      const url = editingId ? `${API}/commandes-clients/${editingId}` : `${API}/commandes-clients`;
      const res = await fetch(url, { method: editingId ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      if (!res.ok) throw new Error("Erreur enregistrement"); await loadData(); resetForm(); setShowForm(false);
    ...
    return (
      <div className="space-y-4 md:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <form onSubmit={handleSubmit} className="bg-white border-2 border-gray-200 rounded-sm p-4 md:p-6 space-y-6">
          <div className="flex items-center justify-between gap-3"><h2 className="text-sm font-semibold text-gray-800">{editingId ? "Modifier la commande" : "Nouvelle commande"}</h2>{editingId && <button type="button" onClick={handleCancel} className="text-xs text-gray-400 hover:text-gray-600 inline-flex items-center gap-1 cursor-pointer"><X className="w-3 h-3" /> Annuler</button>}</div>
          <div className="space-y-4 border-b border-gray-100 pb-5">
            <h3 className="text-xs font-semibold text-gray-700 flex items-center gap-1.5"><ShoppingCart className="w-3.5 h-3.5 text-[#f2762b]" /> Informations</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              <div className="space-y-1.5"><label className="text-xs text-gray-500">Numéro *</label><input name="numero" value={form.numero} onChange={handleInputChange} required placeholder="CMD-001" className={inputBase} /></div>
              <div className="space-y-1.5"><label className="text-xs text-gray-500">Date *</label><input name="date_commande" type="date" value={form.date_commande} onChange={handleInputChange} required className={inputBase} /></div>
              ...
              <div className="space-y-1.5"><label className="text-xs text-gray-500">Devis (optionnel)</label><select name="devis_id" value={form.devis_id} onChange={handleInputChange} className={inputBase}><option value="">Aucun</option>{devis.filter(d => !form.client_id || d.client_id === form.client_id).map(d => <option key={d.id} value={d.id}>{d.numero}</option>)}</select></div>
              <div className="space-y-1.5"><label className="text-xs text-gray-500">Statut</label><select name="statut" value={form.statut} onChange={handleInputChange} className={inputBase}><option value="">Sélectionner</option><option value="brouillon">Brouillon</option><option value="envoyé">Envoyé</option><option value="accepté">Accepté</option><option value="refusé">Refusé</option><option value="annulé">Annulé</option></select></div>
            </div>
          </div>
  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col-reverse md:flex-row items-stretch md:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><input type="text" placeholder="Rechercher une commande..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className={`${inputBase} pl-10 h-10`} /></div>
        <button type="button" onClick={handleNewItem} className="inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-sm bg-[#f2762b] hover:bg-[#d96521] text-xs font-semibold text-white transition-colors cursor-pointer"><Plus className="w-3.5 h-3.5" /> Nouvelle commande</button>
      </div>
      {error && <div className="rounded-sm border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-600">{error}</div>}
      <div className="bg-white border-2 border-gray-200 rounded-sm overflow-hidden text-[#414141]"><div className="overflow-x-auto"><table className="min-w-full text-xs"><thead className="bg-gray-50 text-gray-500"><tr><th className="px-3 md:px-5 py-3 text-left font-semibold">Numéro</th><th className="px-3 md:px-5 py-3 text-left font-semibold">Date</th><th className="px-3 md:px-5 py-3 text-left font-semibold">Client</th><th className="px-3 md:px-5 py-3 text-left font-semibold">Statut</th><th className="px-3 md:px-5 py-3 text-right font-semibold">Montant</th><th className="px-3 md:px-5 py-3 text-right font-semibold">Actions</th></tr></thead>
        <tbody>{loading ? <tr><td colSpan={6} className="px-5 py-6 text-center text-gray-400">Chargement...</td></tr> : filtered.length === 0 ? <tr><td colSpan={6} className="px-5 py-6 text-center text-gray-400">{searchQuery ? "Aucun résultat." : "Aucune commande."}</td></tr> : filtered.map(d => { const st = statusConfig[d.statut] || statusConfig.brouillon; return (
