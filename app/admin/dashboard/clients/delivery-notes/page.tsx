...
export default function ClientDeliveryNotesPage() {
  const [items, setItems] = useState<BLClient[]>([]);
  ...
  const [form, setForm] = useState({ numero: "", date_bl: "", client_id: "", commande_id: "", statut: "" });
  const [lignes, setLignes] = useState<LigneForm[]>([]);

  const resetForm = () => {
    setEditingId(null);
    setForm({ numero: "", date_bl: "", client_id: "", commande_id: "", statut: "" });
    setLignes([]);
  };
  ...
  const handleEdit = (d: BLClient) => {
    ...
    const withLines = d as BLClient & { lignes?: LigneDoc[] };
    setLignes(
  ...
  const handleDelete = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/bl-clients/${id}`, { method: "DELETE" });
  ...
  const handlePrint = (d: BLClient) => {
    ...
    let y = 20;
    doc.setFontSize(14);
    doc.text("Bon de livraison client", 20, y);
    y += 10;
    doc.setFontSize(10);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId ? `${API_BASE_URL}/bl-clients/${editingId}` : `${API_BASE_URL}/bl-clients`;
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-white">BL client</h1>
            <p className="text-xs text-gray-400">Créer et gérer les bons de livraison clients.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <select name="commande_id" value={form.commande_id} onChange={handleInputChange} className="w-full rounded-md bg-[#3a3a3a] border border-white/10 px-3 py-1.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-[#f2762b]">
              ...
              {commandes.filter((c) => !form.client_id || c.client_id === form.client_id).map((c) => (
