...
type DevisFournisseur = { id: string; numero: string; fournisseur_id: string };
...
export default function SupplierOrdersPage() {
  const [items, setItems] = useState<CommandeFournisseur[]>([]);
  ...
  const [form, setForm] = useState({ numero: "", date_commande: "", fournisseur_id: "", devis_id: "", statut: "" });
  const [lignes, setLignes] = useState<LigneForm[]>([]);

  const resetForm = () => {
    setEditingId(null);
    setForm({ numero: "", date_commande: "", fournisseur_id: "", devis_id: "", statut: "" });
    setLignes([]);
  };
  ...
  const loadData = async () => {
    ...
    try {
      const [r1, r2, r3, r4] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/devis-fournisseurs`),
      ]);
  ...
  const handleEdit = (d: CommandeFournisseur) => {
    setEditingId(d.id);
    setForm({
      numero: d.numero,
    ...
    const withLines = d as CommandeFournisseur & { lignes?: LigneDoc[] };
    setLignes(
  ...
  const handleDelete = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/commandes-fournisseurs/${id}`, { method: "DELETE" });
  ...
  const handlePrint = (d: CommandeFournisseur) => {
    const withLines = d as CommandeFournisseur & { lignes?: LigneDoc[] };
    const doc = new jsPDF();
    let y = 20;
    doc.setFontSize(14);
    doc.text("Commande fournisseur", 20, y);
    y += 10;
    doc.setFontSize(10);
    ...
    doc.save(`commande-fournisseur-${d.numero}.pdf`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId ? `${API_BASE_URL}/commandes-fournisseurs/${editingId}` : `${API_BASE_URL}/commandes-fournisseurs`;
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-white">Commandes fournisseur</h1>
            <p className="text-xs text-gray-400">Créer et gérer les commandes fournisseurs.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <select name="devis_id" value={form.devis_id} onChange={handleInputChange} className="w-full rounded-md bg-[#3a3a3a] border border-white/10 px-3 py-1.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-[#f2762b]">
              ...
              {devis.filter((d) => !form.fournisseur_id || d.fournisseur_id === form.fournisseur_id).map((d) => (
