...
export default function SupplierQuotesPage() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const [devisRes, fournisseursRes, produitsRes] = await Promise.all([
      ...
      if (!devisRes.ok) {
        throw new Error("Impossible de charger les devis fournisseurs");
      }
      ...
      const devisData: DevisFournisseur[] = await devisRes.json();
      const fournisseursData: Fournisseur[] = await fournisseursRes.json();
  ...
  const handleEdit = (d: DevisFournisseur) => {
    setEditingId(d.id);
    setForm({
      numero: d.numero,
    ...
    const dWithLines = d as DevisFournisseur & { lignes?: DevisLigne[] };
  ...
  const handleDelete = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/devis-fournisseurs/${id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(
          data?.message || "Impossible de supprimer le devis fournisseur",
        );
      }
      await loadData();
      if (editingId === id) {
  ...
  const handlePrint = (d: DevisFournisseur) => {
    const devisWithLines = d as DevisFournisseur & { lignes?: DevisLigne[] };
    ...
    doc.setFontSize(14);
    doc.text("Devis fournisseur", marginLeft, currentY);
    currentY += 8;
    ...
    doc.save(`devis-fournisseur-${d.numero}.pdf`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId
        ? `${API_BASE_URL}/devis-fournisseurs/${editingId}`
        : `${API_BASE_URL}/devis-fournisseurs`;
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(
          data?.message || "Impossible d'enregistrer le devis fournisseur",
        );
      }
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              Créer et gérer les devis pour vos fournisseurs.
            </p>
          </div>
        </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-gray-300" htmlFor="fournisseur_id">
              Fournisseur
            </label>
            <select
              id="fournisseur_id"
