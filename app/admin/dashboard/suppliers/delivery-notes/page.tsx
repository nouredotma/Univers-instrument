...
type CommandeFournisseur = { id: string; numero: string; fournisseur_id: string };
...
export default function SupplierDeliveryNotesPage() {
  const [items, setItems] = useState<BLFournisseur[]>([]);
  ...
  const [form, setForm] = useState({ numero: "", date_bl: "", fournisseur_id: "", commande_id: "", statut: "" });
  const [lignes, setLignes] = useState<LigneForm[]>([]);

  const resetForm = () => {
    setEditingId(null);
    setForm({ numero: "", date_bl: "", fournisseur_id: "", commande_id: "", statut: "" });
    setLignes([]);
  };
  ...
  const loadData = async () => {
    ...
    try {
      const [r1, r2, r3, r4] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/commandes-fournisseurs`),
      ]);
  ...
  const handleEdit = (d: BLFournisseur) => {
    setEditingId(d.id);
    setForm({
      numero: d.numero,
      date_bl: d.date_bl?.slice(0, 10) ?? "",
      fournisseur_id: d.fournisseur_id,
      commande_id: d.commande_id ?? "",
      statut: d.statut || "",
    });
    const withLines = d as BLFournisseur & { lignes?: LigneDoc[] };
    setLignes(
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Supprimer ce BL ?")) return;
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/bl-fournisseurs/${id}`, { method: "DELETE" });
  ...
  const handlePrint = (d: BLFournisseur) => {
    const withLines = d as BLFournisseur & { lignes?: LigneDoc[] };
    const doc = new jsPDF();
    let y = 20;
    doc.setFontSize(14);
    doc.text("Bon de livraison fournisseur", 20, y);
    y += 10;
    doc.setFontSize(10);
    doc.text(`Numéro: ${d.numero}`, 20, y);
    y += 6;
    doc.text(`Date: ${d.date_bl?.slice(0, 10) || "-"}`, 20, y);
    y += 6;
    ...
    doc.save(`bl-fournisseur-${d.numero}.pdf`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId ? `${API_BASE_URL}/bl-fournisseurs/${editingId}` : `${API_BASE_URL}/bl-fournisseurs`;
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-md bg-gray-800 flex items-center justify-center">
            <Truck className="w-5 h-5 text-[#f2762b]" />
          </div>
          <div>
            <h1 className="text-lg font-semibold text-white">BL fournisseur</h1>
            <p className="text-xs text-gray-400">Créer et gérer les bons de livraison fournisseurs.</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="bg-[#2c2c2c] border border-white/5 rounded-xl p-4 md:p-5 space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-sm font-semibold text-white">{editingId ? "Modifier le BL" : "Nouveau BL"}</h2>
          {editingId && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-gray-300">Date BL</label>
            <input name="date_bl" type="date" value={form.date_bl} onChange={handleInputChange} required className="w-full rounded-md bg-[#3a3a3a] border border-white/10 px-3 py-1.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-[#f2762b]" />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-gray-300">Commande (optionnel)</label>
            <select name="commande_id" value={form.commande_id} onChange={handleInputChange} className="w-full rounded-md bg-[#3a3a3a] border border-white/10 px-3 py-1.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-[#f2762b]">
      ...
      <div className="bg-[#2c2c2c] border border-white/5 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-white">Liste des BL</h2>
          ...
          <table className="min-w-full text-xs">
            ...
            <tbody>
              {items.length === 0 && !loading ? (
                <tr>
                  <td colSpan={5} className="px-3 py-6 text-center text-gray-400">Aucun BL.</td>
                </tr>
              ) : (
                items.map((d) => (
                  <tr key={d.id} className="border-t border-white/5 hover:bg-white/5">
                    <td className="px-3 py-2 text-gray-100">{d.numero}</td>
                    <td className="px-3 py-2 text-gray-100">{d.date_bl?.slice(0, 10)}</td>
